...
export function startAutosave(): () => void {
  ...
  const unsubscribe = useAppStore.subscribe((state) => {
    // Pins — save any newly-added or changed pin.
    ...
    // Layers — save when style, visibility, name, projectId, or feature
    // data changes. (We don't deep-equal the FeatureCollection; instead
    // we compare its reference, and rely on callers swapping the
    // reference when they edit features.)
    if (state.layers !== prevLayers) {
      ...
      for (const layer of state.layers) {
        ...
        if (
          !old ||
          !shallowEqual(old.style, layer.style) ||
          old.visible !== layer.visible ||
          old.name !== layer.name ||
          old.projectId !== layer.projectId ||
          old.data !== layer.data
        ) {
    ...
    // Projects — save when any project's name, visibility, or
    // updatedAt has changed since last snapshot.
    if (state.projects !== prevProjects) {
      const prevById = new Map(prevProjects.map((p) => [p.id, p]));
      for (const project of state.projects) {
        const old = prevById.get(project.id);
        if (
          !old ||
          old.name !== project.name ||
          old.visible !== project.visible ||
          old.updatedAt !== project.updatedAt
        ) {
          saveProject(project).catch(noop);
        }
      }
